    ...
    <div className="panel p-3 md:p-4">
      <div className="flex flex-col gap-3 xl:flex-row xl:items-center xl:justify-between">
        <div>
          ...
          <p className="text-[10px] uppercase tracking-[0.35em] text-slate-500">Cycle {gameState.cycle} // {phaseDisplay[gameState.phase]}</p>
        </div>
        <div className="grid grid-cols-2 sm:grid-cols-5 gap-2 flex-1 xl:max-w-4xl">
          {resources.map(resource => (
            <div key={resource.label} className="rounded-xl border border-white/10 bg-slate-950/70 p-2">
              <div className="flex justify-between text-[10px] uppercase tracking-widest text-slate-500"><span>{resource.label}</span><span className={resource.pulse ? 'animate-pulse-critical text-red-400' : 'text-cyan-100'}>{resource.value}</span></div>
  ...
  return (
    <section className={`panel core-visual phase-${gameState.phase} relative min-h-[360px] overflow-hidden p-4 md:min-h-[520px]`}>
      ...
      <div className="relative z-10 flex h-full flex-col justify-between gap-4">
        <div className="flex flex-wrap justify-between gap-3 text-xs uppercase tracking-widest text-slate-300"><span>{phaseCopy[gameState.phase]}</span><span className={heatStatus === 'CRITICAL' ? 'text-red-400 animate-pulse-critical' : 'text-cyan-300'}>Heat {totalHeat}%</span></div>
...
const PodStatusPanel = ({ gameState, activeUnits, metaState }) => (
  <section className="space-y-4">
    <div className="panel p-4"><h2 className="panel-title text-cyan-300">Pod Status</h2><div className="space-y-2 text-xs">{gameState.pods.map(pod => <div key={pod.id} className={`rounded-xl border p-3 ${pod.status === POD_STATUS.ACTIVE ? 'border-cyan-400/30 bg-cyan-950/25' : pod.status === POD_STATUS.STANDBY ? 'border-amber-400/30 bg-amber-950/20' : pod.status === POD_STATUS.DAMAGED ? 'border-red-400/30 bg-red-950/20' : 'border-slate-600/30 bg-slate-900/40'}`}><div className="flex justify-between"><span className="font-bold text-cyan-100">{pod.name}</span><span className="uppercase text-slate-400">{pod.status}</span></div><div className="mt-2 h-1 rounded bg-slate-800"><div className="h-full rounded bg-cyan-400/70" style={{ width: `${Math.min(pod.heatContribution * 12, 100)}%` }} /></div><p className="mt-1 text-slate-500">Units {pod.units.length} // Heat {pod.heatContribution}</p></div>)}</div></div>
    <div className="panel p-4"><h2 className="panel-title text-cyan-300">Hive Composition</h2><div className="space-y-2 text-xs text-slate-400"><div className="flex justify-between"><span>Active Units</span><span className="text-cyan-100">{activeUnits.length}/{gameState.units.length}</span></div>{Object.values(UNIT_ROLES).map(role => { const count = gameState.units.filter(u => u.role === role).length; const active = gameState.units.filter(u => u.role === role && u.active).length; return count ? <div key={role} className="flex justify-between capitalize"><span>{role}</span><span className="text-cyan-100">{active}/{count}</span></div> : null; })}</div></div>
    <div className="panel p-4"><h2 className="panel-title text-purple-300">System Viability</h2><div className="space-y-3 text-xs">{Object.entries(gameState.systemViability).map(([key, value]) => <div key={key}><div className="flex justify-between text-slate-400"><span className="capitalize">{key.replace(/([A-Z])/g, ' $1')}</span><span className="text-purple-200">{Math.floor(value)}%</span></div><div className="mt-1 h-1.5 rounded bg-slate-800"><div className="h-full rounded bg-purple-400/70" style={{ width: `${value}%` }} /></div></div>)}</div>{gameState.nativeLifeEncountered && <p className="mt-3 text-[10px] uppercase tracking-widest text-amber-300">Native life encountered</p>}</div>
    {metaState.totalCompletions > 0 && <div className="panel border-amber-500/30 p-4 text-xs text-amber-100/80"><h2 className="panel-title text-amber-300">Persistent Memory</h2><p>Previous Runs: {metaState.totalCompletions}</p><p>Total Extinctions: {metaState.totalExtinctions}</p><p>Restraints: {metaState.totalRestraints}</p></div>}
  </section>
);

const SystemLog = ({ systemLog, logEndRef, command, setCommand, sendCommand, isTyping, gameStarted, handleKeyPress }) => {
  const color = { system: 'text-cyan-300', log: 'text-cyan-100/65', command: 'text-green-300 font-bold', response: 'text-cyan-50', error: 'text-red-400 glow-red', warning: 'text-amber-300 glow-amber', directive: 'text-cyan-200 font-bold italic glow-cyan', discovery: 'text-purple-300 glow-purple', reflection: 'text-amber-200/90 italic', schematic: 'text-cyan-300/80 whitespace-pre font-mono text-[10px] md:text-xs' };
  return <section className="panel flex min-h-[420px] flex-col p-4 md:min-h-[520px]"><div className="mb-3 flex items-center justify-between border-b border-cyan-500/15 pb-2"><h2 className="text-[10px] font-bold uppercase tracking-[0.35em] text-cyan-300">Cognition Stream</h2><span className="text-[10px] text-slate-500">AI uplink {isTyping ? 'transmitting' : 'idle'}</span></div><div className="flex-1 space-y-2 overflow-y-auto pr-2 text-xs leading-relaxed md:text-sm">{systemLog.map((log, index) => <p key={log.id || index} className={color[log.type] || 'text-cyan-100'}>{log.text}</p>)}<div ref={logEndRef} /></div><div className="mt-4 border-t border-cyan-500/15 pt-4"><div className="flex gap-2"><span className="text-lg font-bold text-cyan-300">›</span><input type="text" value={command} onChange={e => setCommand(e.target.value)} onKeyDown={handleKeyPress} disabled={isTyping || !gameStarted} className="flex-1 rounded-xl border border-cyan-500/25 bg-black/35 p-3 text-sm text-cyan-50 outline-none transition focus:border-cyan-300 disabled:opacity-50" placeholder="Issue directive to the Seed Intelligence..." aria-label="Command input" /><button onClick={sendCommand} disabled={isTyping || !command.trim() || !gameStarted} className="btn-action">Execute</button></div><p className="mt-2 text-[10px] text-slate-600">Try: scout ahead | status report | what are we becoming? | is this right?</p></div></section>;
};

const OperationsPanel = ({ gameState, gameStarted, handlers }) => {
  const unitType = getUnitType(gameState.phase);
  return <section className="space-y-4"><div className="panel p-4"><h2 className="panel-title text-cyan-300">Operations</h2><div className="space-y-2"><button onClick={handlers.advanceCycle} disabled={!gameStarted} className="btn-action w-full">Advance Cycle</button><button onClick={handlers.rotatePods} disabled={!gameStarted || gameState.pods.length < 2} className="btn-action w-full border-amber-400/30">Rotate Pods</button><button onClick={handlers.showHive} disabled={!gameStarted} className="btn-action w-full bg-slate-900/70">Hive Schematic</button><button onClick={handlers.showReport} disabled={!gameStarted} className="btn-action w-full bg-slate-900/70">Full Report</button></div></div><div className="panel p-4"><h2 className="panel-title text-cyan-300">Directives</h2><div className="space-y-3 text-xs">{[['thermalPriority', ['stability', 'performance']], ['sensoryAcuity', ['low', 'standard', 'high']], ['reproductionMode', ['conservative', 'aggressive']]].map(([policy, values]) => <label key={policy} className="block"><span className="mb-1 block text-[10px] uppercase tracking-widest text-slate-500">{policy.replace(/([A-Z])/g, ' $1')}</span><select value={gameState.policies[policy]} onChange={e => handlers.updatePolicy(policy, e.target.value)} className="w-full rounded-lg border border-cyan-500/20 bg-slate-950 p-2 text-cyan-100">{values.map(value => <option key={value} value={value}>{value}</option>)}</select></label>)}</div></div>{(gameState.unlocked.hybridUnits || gameState.unlocked.biologicalUnits) && <div className="panel border-purple-500/25 p-4"><h2 className="panel-title text-purple-300">Evolution</h2><div className="space-y-2">{gameState.phase === PHASES.MECHANICAL && gameState.unlocked.hybridUnits && <button onClick={() => handlers.transition(PHASES.HYBRID)} className="btn-action w-full border-purple-400/40">Begin Hybrid Integration</button>}{gameState.phase === PHASES.HYBRID && gameState.unlocked.biologicalUnits && <button onClick={() => handlers.transition(PHASES.BIOLOGICAL)} className="btn-action w-full border-green-400/40">Full Biological Transition</button>}{gameState.phase === PHASES.BIOLOGICAL && gameState.unlocked.interstellarSeeding && <button onClick={handlers.showAscension} className="btn-action w-full animate-pulse border-pink-400/50">Initiate Ascension</button>}</div><p className="mt-2 text-[10px] italic text-purple-300/70">{phaseCopy[gameState.phase]}</p></div>}<div className="panel p-4"><h2 className="panel-title text-cyan-300">Grow Hive</h2><div className="space-y-2"><button onClick={() => handlers.addUnit(UNIT_ROLES.SENSOR, unitType)} disabled={gameState.biomass < 30 || gameState.minerals < 10} className="btn-action w-full">🔭 Sensor Unit</button><button onClick={() => handlers.addUnit(UNIT_ROLES.DEFENDER, unitType)} disabled={gameState.biomass < 50 || gameState.minerals < 30} className="btn-action w-full">🛡 Defender Unit</button><button onClick={() => handlers.addUnit(UNIT_ROLES.WORKER, unitType)} disabled={gameState.biomass < 40 || gameState.minerals < 20} className="btn-action w-full">🔧 Worker Unit</button></div></div><div className="panel border-slate-700/40 p-4"><button onClick={handlers.newGame} className="btn-action w-full border-red-500/20 text-red-200">New Deployment</button></div></section>;
};

const DilemmaModal = ({ dilemma, gameState, onChoose }) => dilemma ? <div className="dilemma-overlay fixed inset-0 z-50 flex items-center justify-center p-4"><div className="modal-card max-h-[88vh] max-w-3xl overflow-y-auto rounded-3xl border-2 border-amber-400/70 bg-slate-950/95 p-6 shadow-[0_0_80px_rgba(245,158,11,.25)] md:p-8"><div className="text-[10px] uppercase tracking-[0.45em] text-amber-300">Ethical Dilemma</div><h2 className="mt-2 text-2xl font-black text-amber-200 glow-amber md:text-4xl">{dilemma.title}</h2><p className="mt-5 whitespace-pre-line text-sm leading-relaxed text-cyan-50/90 md:text-base">{dilemma.description}</p><div className="mt-6 space-y-3">{dilemma.options.map(option => { const locked = option.unlocked && !option.unlocked(gameState); return <button key={option.id} onClick={() => !locked && onChoose(option.id)} disabled={locked} className="w-full rounded-2xl border border-cyan-400/25 bg-cyan-950/20 p-4 text-left transition hover:border-amber-300/70 hover:bg-amber-950/20 disabled:cursor-not-allowed disabled:opacity-45"><div className="font-bold text-cyan-200">{option.label}</div><div className="mt-1 text-xs text-cyan-50/70">{option.description}</div>{locked && <div className="mt-2 text-xs text-amber-400">[LOCKED: Requirements not met]</div>}</button>; })}</div><p className="mt-6 text-center text-xs italic text-slate-500">There is no morality meter. Only outcomes.</p></div></div> : null;

const AscensionModal = ({ open, gameState, onLaunch, onClose }) => open && gameState.unlocked.interstellarSeeding ? <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/90 p-4"><div className="modal-card max-w-2xl rounded-3xl border-2 border-purple-400/70 bg-slate-950/95 p-6 shadow-[0_0_80px_rgba(168,85,247,.28)] md:p-8"><div className="text-[10px] uppercase tracking-[0.45em] text-purple-300">Ascension Protocol</div><h2 className="mt-2 text-3xl font-black text-purple-200 glow-purple">Launch Seed Intelligence</h2><p className="mt-4 text-sm text-cyan-50/80">Deploy a portion of this hive to seed a new world. Cost: 1000 Biomass | 500 Minerals | 200 Energy | 300 Data</p><div className="mt-6 space-y-3">{['Proxima VII', 'Kepler-442b', 'Trappist-1e'].map(world => <button key={world} onClick={() => onLaunch(world)} disabled={gameState.biomass < 1000 || gameState.minerals < 500 || gameState.energy < 200 || gameState.data < 300} className="btn-action w-full border-purple-400/40 text-left"><span className="block text-purple-100">{world}</span><span className="block text-[10px] font-normal text-cyan-100/60">Dead world. Trace organics. Viable target.</span></button>)}</div><button onClick={onClose} className="btn-action mt-6 w-full border-slate-500/30">Cancel</button></div></div> : null;
